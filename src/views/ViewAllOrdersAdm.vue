...
const applyFilter = (filterData) => {
    filteredOrders.value = orders.value.filter((order) => {
        return (
            (filterData.selectedStatus === 'All' || order.status === filterData.selectedStatus) &&
            order.price >= filterData.priceRange[0] &&
            order.price <= filterData.priceRange[1] &&
            (!filterData.startDate || new Date(order.date) >= new Date(filterData.startDate)) &&
            (!filterData.endDate || new Date(order.date) <= new Date(filterData.endDate)) &&
            (!filterData.selectedRestaurant || order.restaurant === filterData.selectedRestaurant) &&
            (!filterData.searchQuery || order.customerName.toLowerCase().includes(filterData.searchQuery.toLowerCase()))
        );
    });
};
</script>
